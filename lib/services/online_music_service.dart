...
class OnlineMusicService {
    ...
    try {
      ...
      if (data is Map && data['code'] == 200) {
        ...
        return result.take(20).map((item) {
          ...
          return Song(id: song['id']?.toString() ?? '', title: song['name'] ?? '未知',
            artist: (song['artists'] as List?)?.map((a) => a['name']).join('/') ?? '未知',
            album: song['album']?['name'], albumArt: song['album']?['picUrl'] ?? song['picUrl'],
            url: '', duration: song['duration'] ?? 0, isOnline: true);
        }).toList();
      }
    } catch (_) {}
    return [];
    ...
    try {
      ...
      if (data is Map && data['code'] == 200) {
        ...
        try {
          ...
          if (dData is Map && dData['code'] == 200) { for (final s in dData['songs'] ?? []) coverMap[s['id']] = s['al']?['picUrl']; }
        } catch (_) {}
        return songsData.map((s) => Song(id: s['id']?.toString() ?? '', title: s['name'] ?? '未知',
          artist: (s['artists'] as List?)?.map((a) => a['name']).join('/') ?? '未知',
          album: s['album']?['name'], albumArt: coverMap[s['id']], url: '', duration: s['duration'] ?? 0, isOnline: true)).toList();
      }
    } catch (_) {}
    return [];
  }
  Future<String?> getSongUrl(String songId) async {
    try {
      ...
      final data = resp.data;
      if (data is Map && data['code'] == 200) { final List d = data['data'] ?? []; if (d.isNotEmpty) return d[0]['url']; }
    } catch (_) {}
    try {
